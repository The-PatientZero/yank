...
struct PublicRepositoryTests {
    ...
    @Test("Version 1.2.0 is shared by every application bundle")
    func releaseVersionIsSharedByEveryApplicationBundle() throws {
        let project = try String(
            contentsOf: repositoryRoot.appendingPathComponent("project.yml"),
            encoding: .utf8
        )
        #expect(project.contains(#"MARKETING_VERSION: "1.2.0""#))
    ...
    func testFlightWorkflowPreservesReleaseSecurityBoundary() throws {
        ...
        #expect(workflow.contains("group: testflight-promotion"))
        ...
        #expect(promoteSection.contains("TESTFLIGHT_EXTERNAL_GROUP_ID: ${{ vars.TESTFLIGHT_EXTERNAL_GROUP_ID }}"))
        #expect(!promoteSection.contains("${{ runner.temp }}"))
